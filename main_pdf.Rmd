...
iohmm_generate <- function(T) {
  ...
  x_t <- vector("numeric", T)
  for (t in 1:T) {
    x_t[t] <- rnorm(1, u[t, ] %*% b[z[t], ], sigma[z[t]])
  }
...
plot_inputoutput(x = dataset$x_t, u = dataset$u, z_t = dataset$z)
```

We observe how the chosen values for the parameters affect the generated data. For example, the relationship between the third input $\mat{u}_3$ and the output $\mat{x}_t$ is positive, indifferent and negative for the hidden states $K = 1, 2, 3$ respectively. The true slopes are `r dataset$theta$b[1, 3]`, `r dataset$theta$b[2, 3]` and `r dataset$theta$b[3, 3]`.

```{r iohmm_walkthrough_inputprob, include = TRUE, echo = FALSE, eval = TRUE, fig.height = 9, out.width="\\textwidth"}
plot_inputprob(u = dataset$u, p.mat = dataset$theta$p.mat, z_t = dataset$z)
```
...
fit <- iohmm_fit(K, dataset$u, dataset$x_t)
```
