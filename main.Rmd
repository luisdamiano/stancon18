...
Real-world processes produce observable outputs characterized as signals. These can be discrete or continuous in nature, can embed uncertainty about the measurements and the explanatory model, come from a stationary or non stationary source, among many other variations. These signals are modeled to allow for both theoretical descriptions and practical applications. The model itself can be deterministic or stochastic, in which case the signal is well characterized as a parametric random process whose parameters can be estimated in a well-defined manner.
...
hmm_generate <- function(K, T) {
  ...
  # 3. Observations
  y_t <- vector("numeric", T)
  for (t in 1:T)
    y_t[t] <- rnorm(1, mu_k[z_t[t]], sigma_k[z_t[t]])

  list(y_t = y_t, z_t = z_t,
...
Filtered marginals can be computed recursively by means of the forward algorithm [@baum1967inequality]. Let $\psi_t(j) = p(\mat{y}_t | z_t = j)$ be the local evidence at step $t$ and $\Psi(i, j) = p(z_t = j | z_{t-1} = i)$ be the transition probability. First, the one-step-ahead predictive density is computed
...
\[
\log p(\mat{y}_{1:T} | \mat{\theta}) = \sum_{t=1}^{T}{\log p(\mat{y}_{t} | \mat{y}_{1:t-1})} = \sum_{t=1}^{T}{\log Z_t}.
\]
...
transformed parameters {
  ...
  { // Forward algorithm log p(z_t = j | y_{1:t})
    ...
    unalpha_tk[1] = log(p_1k) + normal_lpdf(y_t[1] | mu_k, sigma_k);
    ...
    for (t in 2:T) {
      for (j in 1:K) { // j = current (t)
        for (i in 1:K) { // i = previous (t-1)
          ...
          accumulator[i] = unalpha_tk[t-1, i] + log(A_ij[i, j]) + normal_lpdf(y_t[t] | mu_k[j], sigma_k[j]);
        }
...
Intuitively, the forward-backward algorithm passes information from left to right and then from right to left, combining them at each node. A straightforward implementation of the algorithm runs in $O(K^2 T)$ time because of the $K \times K$ matrix multiplication at each step. It is well worth noting that the forward and backward procedures, nonetheless the frequent description as two subsequent passes, are not inherently sequential and share no informations. As a result, they could be implemented in parallel.
    ...
    for (tforward in 0:(T-2)) {
      ...
      for (j in 1:K) { // j = previous (t-1)
        for (i in 1:K) { // i = next (t)
          ...
          accumulator[i] = unbeta_tk[t, i] + log(A_ij[j, i]) + normal_lpdf(y_t[t] | mu_k[i], sigma_k[i]);
          }
...
generated quantities {
  ...
  { // Viterbi algorithm
    ...
    for (j in 1:K)
      delta_tk[1, K] = normal_lpdf(y_t[1] | mu_k[j], sigma_k[j]);
    ...
    for (t in 2:T) {
      for (j in 1:K) { // j = current (t)
        ...
        for (i in 1:K) { // i = previous (t-1)
          real logp;
          logp = delta_tk[t-1, i] + log(A_ij[i, j]) + normal_lpdf(y_t[t] | mu_k[j], sigma_k[j]);
...
hmm_init <- function(K, y_t) {
  clasif <- kmeans(y_t, K)
  init.mu <- by(y_t, clasif$cluster, mean)
  init.sigma <- by(y_t, clasif$cluster, sd)
...
```{r hmm_walkthrough_estimate, echo = TRUE, eval = TRUE, cache = TRUE}
set.seed(900)
K        <- 3
T_length <- 500
dataset  <- hmm_generate(K, T_length)
fit      <- hmm_fit(K, dataset$y_t)
```
...
```{r hmm_walkthrough_summary, echo = TRUE, eval = TRUE}
knitr::kable(cbind(
...
data {
  ...
  real y_t[T];                      // output (scalar so far)
...
transformed parameters {
  ...
  { // Evidence (observation likelihood)
    for(t in 1:T) {
      for(j in 1:K) {
        oblik_tk[t][j] = normal_lpdf(y_t[t] | mu_k[j], s_k[j]);
      }
    }
  }
...
iohmm_generate <- function(T) {
  ...
  y_t <- vector("numeric", T)
  for (t in 1:T) {
    y_t[t] <- rnorm(1, u_tm[t, ] %*% b_km[z_t[t], ], sigma_k[z_t[t]])
  }
...
plot_inputoutput(x = dataset$y_t, u = dataset$u_tm, z = dataset$z_t)
```

We observe how the chosen values for the parameters affect the generated data. For example, the relationship between the third input $\mat{u}_3$ and the output $\mat{y}_t$ is positive, indifferent and negative for the hidden states $K = 1, 2, 3$ respectively. The true slopes are `r dataset$theta$b_km[1, 3]`, `r dataset$theta$b_km[2, 3]` and `r dataset$theta$b_km[3, 3]`.
...
```{r iohmm_walkthrough_estimate, echo = TRUE, eval = TRUE, cache = TRUE}
iohmm_fit <- function(K, u_tm, y_t) {
  rstan_options(auto_write = TRUE)
  options(mc.cores = parallel::detectCores())
...
fit <- iohmm_fit(K, dataset$u_tm, dataset$y_t)
```
...
```{r iohmm_walkthrough_summary, echo = TRUE, eval = TRUE}
knitr::kable(cbind(
             unlist(list(dataset$theta$p_1k, 
...
```{r iohmm_walkthrough_stateprobability, echo = TRUE, eval = TRUE, fig.height = 9, out.width="\\textwidth"}
gamma_tk <- extract(fit, pars = 'gamma_tk')[[1]]
...
```{r iohmm_walkthrough_filtered, echo = TRUE, eval = TRUE}
knitr::kable(table(
...
We acknowledge the members of the Stan Development Team for being so passionate about Bayesian statistics and technically unbeatable. Special mentions to Aaron Goodman, Ben Bales and Bob Carpenter for their active participation in the discussions held in Stan forums for [HMM with constraints](http://discourse.mc-stan.org/t/hidden-markov-model-with-constraints/1625/4) and [HMMM](http://discourse.mc-stan.org/t/transversing-up-a-graph-hierarchical-hidden-markov-model/1304/11). Although not strictly related, the discussion remains very valuable for the current tutorial.
...
[^bold_outputs]: The output variable can be univariate or multivariate depending on the choice of model specification. While in the former case the observation at a given time index $t$ is simply a scalar $y_t$, it becomes the vector $\mat{y}_t$ in the latter. In this tutorial, we use a univariate .

As we introduce new equations along with our example, which is univariate, we keep the bold notation to insist that the introduced definitions and properties are also valid in a multivariate context.
