...
suggesting that there is some form of short-term memory to the volatility (standard deviation of returns). Generalized Autoregressive Conditional Heteroskedasticity 
("GARCH") models commonly used to capture this phenomenon and have been widely studied  in the econometrics literature 
...
[-@Haas:2004a; -@Haas:2004b] uses a Hidden Markov Model to switch between two latent GARCH processes. In an extensive empirical comparison, 
...
Under the model, the return at each date is drawn independently from a normal distribution
with mean $\mu$, which we fix to be zero, and a time-varying standard deviation $\sigma_t$ which evolves according
to 
\[\begin{aligned}
\sigma^2_t &= \alpha_0 + \alpha_1 r_{t-1}^2 + \beta_1 \sigma_{t-1}^2 \\
r_t &\sim \mathcal{N}(0, \sigma^2_t)
\end{aligned}\]
...
\[\begin{aligned}
(\sigma^{(1)}_t)^2 &= \alpha^{(1)}_0 + \alpha^{(1)}_1 r_{t-1}^2 + \beta_1^{(1)} \sigma_{t-1}^2 \\
(\sigma^{(2)}_t)^2 &= \alpha^{(2)}_0 + \alpha^{(2)}_1 r_{t-1}^2 + \beta_1^{(2)} \sigma_{t-1}^2 \\
X_{t}|X_{t-1} = k &\sim \text{Categorical}(p_k) \\
r_t|X_t = k &\sim \mathcal{N}(0, (\sigma_t^{(k)})^2)
\end{aligned}\]
...
volatilities $\alpha^{(1)}, \alpha^{(2)}$. We use weak priors on the GARCH coefficients
...
plot(SPY.R, main="S&P 500 Daily Returns")
```
...
Examining the fit, *e.g.* in `ShinyStan` [@sdt2017shinystan], we see that Stan samples efficiently from this model, 
with all $\hat{R}$-statistics below $1.1$ and high $n_{\text{eff}}/n$ ratios: 
```{r eval=FALSE}
library(shinystan)
launch_shinystan(fit)
```

From here, we are able to perform posterior inference, *e.g.*, examining posterior means
of the conditional volatilities from the two GARCH processes
...
plot(garch_posterior_means, main="Posterior Means of Instantaneous Variances")
```
...
plot(garch_posterior_prob, main="Posterior Probability of Low-Vol State")
```
