...
\[
p(\mat{x}_t | z_t = k, \mat{\theta}) = \text{categorical}(\mat{x}_t | \mat{\theta}_k)
\]
...
Smoothing       | $p(z_t `|` \mat{x}_{1:T})$              | $T$ (offline)       | forward-backward         | $O(K^2T)$ \ $O(KT)$ if left-to-right
...
[^bold_outputs]: The output variable can be univariate or multivariate depending on the choice of model specification. While in the former case the observation at a given time index $t$ is simply a scalar $y_t$, it becomes the vector $\mat{x}_t$ in the latter. In this tutorial, we use a univariate .
