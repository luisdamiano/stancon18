...
hmm_generate <- function(K, T) {
  ...
  z[1] <- sample(1:K, size = 1, prob = pi0)
  for (t in 2:T)
  ...
  y <- vector("numeric", T)
  for (t in 1:T)
    y[t] <- rnorm(1, mu_k[z[t]], sigma[z[t]])

  list(y = y, z = z,
       theta = list(pi0 = pi0, A = A,
                    mu = mu, sigma = sigma))
}
```
...
generated quantities {
  ...
  { // Viterbi algorithm
    int a[T, K];       // backpointer to the most likely previous state on the most probable path
...
The variable `delta` is a straightforward implementation of the corresponding equation. `a` is the traceback needed to compute the most probable sequence of states after the most probably final state `zstar` is computed.
...
\[
\LL_T \prop \sum_{i = 1}^{K}{\alpha_T(i)}.
\]
...
knitr::kable(cbind(
             unlist(list(dataset$theta$pi0, 
                        t(dataset$theta$A), 
                        dataset$theta$mu, 
                        dataset$theta$sigma)),
             summary(fit,
                  pars = c('pi0', 'A', 'mu_k', 'sigma'),
...
# plot_stateprobability(alpha, gamma, 0.8, dataset$z)
```
...
The adaptation of the data and parameters blocks is straightforward: we add the number of input variables `M`, the array of input vectors `u`, the regressors `b` and the residual standard deviation `s_k`.
...
transformed parameters {
  ...
  { // Evidence (observation likelihood)
    for(t in 1:T) {
      for(j in 1:K) {
        logoblik[t][j] = normal_lpdf(y[t] | mu[j], s_k[j]);
      }
    }
  }
...
iohmm_generate <- function(T) {
  ...
  # 2. Hidden path
  z <- vector("numeric", T)
  z[1] <- sample(x = 1:K, size = 1, replace = FALSE, prob = pi0)
  for (t in 2:T) {
...
[^bold_outputs]: The output variable can be univariate or multivariate depending on the choice of model specification. An observation at a given time index $t$ is a scalar $y_t$ and a vector $\mat{y}_t$ respectively. As we introduce definitions and properties along an example based on a univariate series, we keep the bold notation to remind that the equations are valid in a multivariate context as well.
